...
function Window(socket) {
  ...
  xButton.title = 'Close'
  xButton.className = 'button close-button';

  minButton = document.createElement('div');
  minButton.className = 'button min-button';

  maxButton = document.createElement('div');
  maxButton.title = 'Maximize'
  maxButton.className = 'button max-button';
...
Window.prototype.bind = function() {
  ...
  on(el, 'mousedown', function(ev) {
    if (ev.target === xButton) {
      return self.destroy();
    };

    if (ev.target === maxButton) {
      return self.maximize();
    };

    if (ev.target !== title && ev.target !== bar) return;
